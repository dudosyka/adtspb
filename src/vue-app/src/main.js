...
extend('date', {
  ...
  validate: function (value) {
    const input = value.match(/^[0-9]{2}-[0-9]{2}-[0-9]{4}$/) !== null // тоже поменять

    const __data = value.split('-')
    const year = parseInt(__data[2]) // 0
    const month = parseInt(__data[1]) // 1
    const day = parseInt(__data[0]) // 2

    const validation1 = year >= 1000 && year <= 9999
    const validation2 = month >= 1 && month <= 12
    const validation3 = day >= 1 && day <= 31

    const year_validation = ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)

    const special = (month == 4 || month == 6 || month == 9 || month == 11) ? day <= 30 : true // месяцы, где нет 31 дня
    const special1 = (!year_validation && month == 2) ? day <= 28 : true // февраль
    const special2 = (year_validation && month == 2) ? day <= 29 : true // високосный год
...
extend('kid_bdate', {
  ...
  validate: function (value) {
    ...
    const age = getAge(year + '-' + month + '-' + day + ' 00:00:00')
    if (age >= 6 && age <= 18) { return true }
